...
+ (NSString *)getUserAgent {
    return [NSString stringWithFormat:@"DCallingiPhone/%@ (DCalling/%s; Apple %@/%@)",
...
- (BOOL)migrateChatDBIfNeeded:(LinphoneCore*)lc {
    ...
    if( shouldMigrate ){
        ...
        if( !identity ){
            identity = "sip:unknown@sip.dcalling.de";
        }
...
static void linphone_iphone_call_state(LinphoneCore *lc, LinphoneCall* call, LinphoneCallState state,const char* message) {
    NSLog(@"state Prashant %s - %u", message, state);
    NSString *gg = [[[NSString alloc]initWithUTF8String:message]autorelease];
    if(![gg isEqualToString:@"Incoming call"])
...
+ (void)kickOffNetworkConnection {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        CFStreamCreatePairWithSocketToHost(NULL, (CFStringRef)@"192.168.0.200"/*"dcalling.de"*/, 15000, nil, &writeStream);
...
void networkReachabilityCallBack(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void* nilCtx){
	...
	if (theLinphoneCore != nil) {
		...
		if ((flags == 0) || (flags & networkDownFlags)) {
			...
			BOOL isWifiOnly = lp_config_get_int(lLinphoneMgr.configDb, LINPHONERC_APPLICATION_KEY, "wifi_only_preference",TRUE);
            if (!ctx || ctx->testWWan)
                newConnectivity = flags & kSCNetworkReachabilityFlagsIsWWAN ? wwan:wifi;  // changed by prashant none to wwan
            else
			...
			if (lLinphoneMgr.connectivity != newConnectivity) {
				...
				[LinphoneLogger logc:LinphoneLoggerLog format:"Network connectivity changed to type [%s]",(newConnectivity==wifi?"wifi":"wwan")]; // prashant change none to wwan
			}
...
- (void)finishCoreConfiguration {
    ...
    linphone_core_set_user_agent(theLinphoneCore,"DCallingiPhone",
    ...
    if( [self lpConfigBoolForKey:@"avpf_migration_done" forSection:@"app"] == FALSE ){
        ...
        while(proxies){
            ...
            if( addr && strstr(addr, "sip.dcalling.de") != 0 ){
